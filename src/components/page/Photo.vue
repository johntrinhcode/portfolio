  ...
  <div class="w-full h-full bg-black">
    <transition name="fade" mode="out-in" id="photo" class="w-full h-full">
      <div
        v-if="currentItem == 0"
        :key="0"
        class="flex md:flex-row w-screen h-full overflow-scroll"
      >
        <v-lazy-image :src="rows[currentItem][0]" class="md:w-1/3 object-cover"></v-lazy-image>
        <v-lazy-image :src="rows[currentItem][1]" class="md:w-1/3 object-cover"></v-lazy-image>
        <v-lazy-image :src="rows[currentItem][2]" class="md:w-1/3 object-cover"></v-lazy-image>
      </div>
      <div
      ...
      <div v-else :key="2" class="flex md:flex-row w-screen h-full overflow-scroll">
        <v-lazy-image :src="rows[currentItem][0]" class="md:w-1/3 object-cover"></v-lazy-image>
        <v-lazy-image :src="rows[currentItem][1]" class="md:w-1/3 object-cover"></v-lazy-image>
        <v-lazy-image :src="rows[currentItem][2]" class="md:w-1/3 object-cover"></v-lazy-image>
      </div>
